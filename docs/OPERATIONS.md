# sm-notification-center（通知中心）运维手册

## 1. 服务概述与依赖

sm-notification-center 为 SM 平台通知中心，监听容器端口 8010。
...
1. `helm history sm-notification-center` 查看版本。
2. `helm rollback sm-notification-center <上一版本>`。
...
- 日志排查：`kubectl logs -l app.kubernetes.io/instance=sm-notification-center` 或 Loki 按 job=sm-notification-center 检索。
